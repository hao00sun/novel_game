...
> **适用项目：Novel World**
>
> **文档版本：v0.2.2（新增半结构化输入与 Intent 解析演进原则，保留概率 NPC 生成资产原则，并整合 v0.3 API 实测问题）**
>
...
2026.8.31：
1、当前仅仅有关于整个项目的想象还不清楚这个项目应该有哪些组件组成（agent？tool？skill？rag？）
2、每个部分的组件实现的自由度还很低，甚至不如传统游戏，没有体现出llm的优势，希望能增加“自动增加功能”的功能，但这种自动增加的功能还要设置合理的审核，如果审核也能自动就好了
3、还没有弄清楚各个组件的生态位（比如什么是agent，skill，tool，为什么要使用它们，他们应该起到怎样的作用？能力边界又应该是什么？每个组件应该使用什么东西？产出什么东西？）
9.16
既然需要将玩家的行为进行分类，可不可以在玩家行动之前先提供各种选项确定要做什么？这样做会出现哪些问题？可能会导致单元化？不能承接复杂意图或动作？搜集足够的数据去训练一个小网络专门做这个事儿，还是调用api负责？
进一步讲，什么任务时候专门训练一个小网络？什么任务适合调用api？
9.17
要清楚什么适合程序，什么适合ai，能量化，能规律的都用程序，不要想“懒得设计”就ai了
需要一个统一的概率/随机机制，但不要给世界所有可能事件逐个写概率；应该先用规则和 Agent 缩小候选空间，再由分层概率模型对真正有意义的不确定结果进行抽样

项目补充优先级排序
| 优先级      | 组件                                       | 当前状态      | 为什么重要                    |
| -------- | ---------------------------------------- | --------- | ------------------------ |
| **P0**   | **Action System（行动系统）**                  | ❌         | 让复杂自然语言真正变成可裁决动作         |
| **P0**   | **Entity / Item System（实体/物品系统）**        | ❌         | 世界现在基本只有人物和地点            |
| **P0**   | **Resolver v2（通用裁决系统）**                  | ⚠️ 很初级    | 从硬编码 move/talk 升级成通用因果裁决 |
| **P0**   | **Probability / Uncertainty（概率与不确定性）**   | 📁 占位     | 决定“怎么成功/怎么失败”            |
| **P1**   | **Time System（时间系统）**                    | 📁 占位     | 让行为真正消耗世界时间              |
| **P1**   | **Event System（事件系统）**                   | 📁 占位     | 世界变化从字符串日志升级成结构化事件       |
| **P1**   | **Knowledge / Evidence / Memory（认知与记忆）** | ⚠️ 初级     | 解决 NPC 为什么知道、为什么相信       |
| **P1**   | **NPC Autonomy（NPC 自主行动）**               | ❌         | 让 NPC 不再只在玩家搭话时活过来       |
| **P2**   | Rumor / Relationship / Faction           | 📁 部分资产已有 | 社会模拟层                    |
| **P2**   | Economy / Resource Pressure              | ❌         | 粮价、商路、流民等宏观联动            |
| **工程基础** | TurnTrace / Replay / Debug               | ⚠️ 很初级    | 后面系统复杂后非常重要              |
...
UI 可以根据当前地点、可见对象和世界状态动态提供：

```text
交谈 / 观察 / 移动 / 交易 / 使用物品 / 冲突 / 其他行动
```

这些选项是高频意图的快捷入口和 `intent_hint`，不是世界规则，也不是合法动作全集：

```text
UI Option ≠ World Rule
```

玩家始终可以直接输入自由语言；未显示在选项中的合理行动仍应进入理解与裁决流程。
...
```text
Player Input + Optional Intent Hint
→ Intent / Action IR
→ Action Proposal
→ World Resolver
→ State Change
...
其中 `Action IR` 是可替换解析器之前的稳定中间表示；复合行为可包含多个 `steps`、协作对象和隐藏意图，但只有被 Tool 和 Resolver 验证通过的部分才能成为 Proposal 或 State Change。第一阶段只实现少量可验证的 Compound Intent，不因表达能力不足而让 LLM 直接补写结果。

### Intent 解析演进

当前优先使用 API 探索并稳定 `Action IR` Schema，而不是过早训练小模型。演进顺序：

```text
V0  API → 结构化 Intent / Action IR
V1  明显高频输入走 Rule，其余走 API
V2  Schema 稳定且有经过抽查的数据后，小模型高置信识别；低置信回退 API
```

小模型负责高频、输入输出固定的识别；LLM 负责开放式语义解析与复杂复合意图；Program / Resolver 始终负责裁决。只有输入与输出 Schema 稳定、调用频率高、数据充足，且 API 的成本或延迟已成为问题时，才考虑训练小模型。
